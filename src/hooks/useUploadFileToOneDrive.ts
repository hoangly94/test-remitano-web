...
const useUploadFileToOneDrive = () => {
    const [data, setData] = useState();
    const uploadFile = getUrl(setData);
    return {
        data,
        uploadFile,
    }
...
const getUrl = (setData) => async (file: any) => {
    const Authorization = 'Bearer ' + (process.env.ACCESS_TOKEN_COOKIE && cookies.get(process.env.ACCESS_TOKEN_COOKIE));

    const createUploadSessionUrl = `${graphUrl}/me/drive/root:/committee/files/${file.name?.trim()}://createUploadSession`;
    ...
    while ([200, 201, 202].includes(resp.status)) {
        if (await !resp.data?.nextExpectedRanges?.length) {
            await setData(resp);
            return resp;
        }
...
export default useUploadFileToOneDrive